...
int GetPvLine(const int depth, S_BOARD *pos) {
	...
	int count = 0;
	#pragma acc parallel loop private(move, count) shared(pos)
...
void ClearHashTable(S_HASHTABLE *table) {
    //parallelize - parallel clearing of hash table entries
    S_HASHENTRY *tableEntry;
	#pragma acc parallel loop private(tableEntry) shared(table)
...
void InitHashTable(S_HASHTABLE *table, const int MB) {
    //parallelize - parallel initialization of hash table
