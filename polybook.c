...
U64 PolyKeyFromBoard(const S_BOARD *board) {
	...
	int polyPiece = 0;
	int offset = 0;
	#pragma acc parallel loop private(sq, piece, polyPiece, rank, file, offset) shared(board)
	for(sq = 0; sq < BRD_SQ_NUM; ++sq) {
		...
		if(piece != EMPTY && piece != OFFBOARD) {
			...
			rank = RanksBrd[sq];
			file = FilesBrd[sq];
			offset = polyPiece * 64 + rank * 8 + file;
...
int GetBookMove(S_BOARD *board) {
	...
	#pragma acc parallel loop private(entry) shared(entries, NumEntries, polyKey)
